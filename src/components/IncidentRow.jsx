...
function IncidentRow({ incident, isSelected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3 text-sm text-white">{incident.company}</td>
      ...
      <td className="px-4 py-3">
        <span
          className={`rounded border px-2 py-0.5 text-xs font-medium ${severity.badge}`}
        >
          {severity.label}
        </span>
      </td>
      <td className="px-4 py-3">
        <span
          className={`rounded border px-2 py-0.5 text-xs font-medium ${status.badge}`}
        >
          {status.label}
        </span>
      </td>
